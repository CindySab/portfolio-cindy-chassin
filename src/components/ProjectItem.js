...
const ProjectItem = ({ title, type, summary, img, link, github, width, height}) => {
    return (
        <article className='h-[550px] w-full flex items-center justify-between xl:flex-col rounded-3xl border border-solid bg-dark/90 p-12 lg:p-8 xs:p-4'>
            <Link href={link} target="_blank" className='w-1/2 cursor-pointer overflow-hidden rounded-lg xl:w-full xl:mb-4 xs:mb-0'>
            ...
            <div className='w-1/2 flex flex-col items-start justify-between pl-6 xl:w-full xl:pl-2'>
                <span className='text-primaryDark font-medium text-xl xl:text-md xs:text-sm'>{type}</span>
                <Link href={link} target="_blank" className='hover:underline underline-offset-2 text-light'>
                    <h2 className='my-2 w-full text-left text-4xl font-bold text-light xl:text-2xl xs:text-xl'>{title}</h2>
                </Link>
                <p className='my-2 font-medium text-light xl:text-sm xs:text-xs'>{summary}</p>
